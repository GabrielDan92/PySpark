...
if presentationMode:
    print("\nCalculate the trip duration for each bus stop, converting each string array element to timestamp on the fly:")
    ...
    for i in range(maxBusStops -1):
        ...
        tripsDF = tripsDF.withColumn(f"duration_in_h_{str(stopName)}", lit(to_timestamp(tripsDF.triptimes[stopNumber]) - to_timestamp(tripsDF.triptimes[i])))
